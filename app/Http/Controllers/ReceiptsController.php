...
class ReceiptsController extends Controller
{
    public function ViewReceipts(){
        ...
        ->get();
    return view('receipts_admin.view_receipts', $items);
    }

   
    public function AddReceipts(){
    	return view('receipts_admin.add_receipts');
    }
    ...
    public function StoreReceipts(Request $request){
        ...
        Receipts::where('name', $request->name)->get()->each(function ($receipt) {
    	...
    	return redirect()->route('receipts_admin.receipt.view')->with($notification);

    }  // End Method

    public function DetailsReceipts($name){
        $receipts = Receipts::where('name', $name)->get();
        $total_amount = $receipts->sum('amount');
        $url = route('receipts_admin.receipt.details', ['name' => Str::slug($name)]);
        $qrCode = QrCode::size(120)->generate($url);
        return view('receipts_admin.details_receipts', compact('receipts', 'total_amount', 'name', 'qrCode','url'));
    }


    public function deleteReceipt($name)
{
    Receipts::where('name', $name)->delete();
...
{
    $data = Receipts::where('name', $name)->first();
    return view('receipts_admin.edit_receipts', compact('data'));
}

public function update(Request $request, $name)
{
    $data = Receipts::where('name', $name)->firstOrFail();
    ...
    public function print($name)
{
    ...
    $url = route('receipts_admin.receipt.details', ['name' => Str::slug($name)]);
        $qrCode = QrCode::size(120)->generate($url);
    $total_amount = $receipts->sum('amount');
    return view('receipts_admin.print_receipt', compact('receipts', 'total_amount','url','qrCode'));
